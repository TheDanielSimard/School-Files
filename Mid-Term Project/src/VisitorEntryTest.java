...
class VisitorEntryTest {
		...
		private final String NAME = "JarJarBinks";
		private VisitorEntry logEntry, logEntryFrozen, entryBefore, entryAfter;
	
		
		@BeforeEach
		void setUp() throws Exception {
			 entryBefore = new VisitorEntry(Instant.now().toString(), NAME);
			 // Let a little time pass
			 Thread.sleep(100);
			 logEntry =  new VisitorEntry(Instant.now().toString(),NAME);
			 logEntryFrozen =  new VisitorEntry(Instant.now().toString(),NAME);
			 // Let a little more time pass
			 Thread.sleep(100);
			 entryAfter = new VisitorEntry(Instant.now().toString(), NAME);
			 logEntryFrozen.freeze(FREEZE_CODE);
		}
/*
 * Test class features specific to VisitorEntry.
*/
		
		@Test
		@DisplayName("Test signOut behavior")
		void testSignOut() {
			String logString;
			assertTrue(logEntry.signOut());
			logString = logEntry.toString();
			assertFalse(logEntry.signOut());
 ...
 */
		...
		void testCopy() {
			...
			VisitorEntry copyLogEntry = new VisitorEntry(logEntry);
